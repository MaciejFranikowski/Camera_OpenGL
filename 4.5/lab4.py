...
def camera_control():
    global theta
    global phi
    theta, phi = abs(theta % 360), abs(phi % 360)
    theta_radians = math.radians(theta)
    phi_radians = math.radians(phi)
    x = scale * math.cos(theta_radians) * math.cos(phi_radians)
    y = scale * math.sin(phi_radians)
    z = scale * math.sin(theta_radians) * math.cos(phi_radians)
    if 90 < phi < 270:
        invert_camera = -1
    else:
        invert_camera = 1
    gluLookAt(x, y, z, 0, 0, 0, 0, invert_camera, 0)
